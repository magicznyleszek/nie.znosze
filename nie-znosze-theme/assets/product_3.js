jQuery( document ).ready(function( $ ) {
    'use strict';
    var p,
        productContainer = $('.shb-product-3'),
        PRODUCT_3 = {
            settings:{
            ...
            init: function(){
                p = this.settings;
                console.log('PRODUCT_3 INIT', p.productContainer);
            ...
            windowLoadHandler: function() {
                ...
                if ( p.isMobile === 1 ) {
                    PRODUCT_3.mobileGallery();
                } else {
                    PRODUCT_3.desktopGallery();
                }

                if ( p.variantsHandler.length ) {
                    Shop.productHandler = p.productContainer.productVariants({
                        productSectionContainer: '.shb-product-3',
            ...
            mobileGallery: function() {
                ...
                initPhotoSwipeFromDOM('.shb-product-3 .shb-main-gallery img', p.mainGallery.data('zoom-ratio'));
            },

            desktopGallery: function() {
                var thumbItem = p.thumbsGallery.find('.shb-thumb-item'),
                    maxThumbsToShow = PRODUCT_3.slidesToShow();
                p.mainGallery.on('init', function(){
                    p.mainGallery.fadeTo("slow", 1);
                    p.mainGallery.find('.shb-gallery-item').css("position", "relative");
                });
                ...
                p.thumbsGallery.imagesLoaded(function(){
                    if (thumbItem.length > maxThumbsToShow) {
                        ...
                        PRODUCT_3.showThumbsGallery(thumbItem);
                    }
                });
            },
