      ...
      cartOptions = {
        settings: {
          ...
          productAddToCartButton: $(
            '.shb-product-list figure figcaption form button[type="submit"], [add-to-cart-button]'
          ),
              ...
              $.each(data.variants, function(k, v) {
                ...
                if (v.id == variantId) {
                  ...
                  if (price != priceRegular) {
                    $('body')
                      .find('[product-price-new]')
                      .find('span')
                      .text(Shop.money_format.replace('{{amount}}', price.toFixed(2)));

                    if ($('body').find('[product-price-old]').length == 0) {
                      $('body')
                        .find('[product-price]:not([cart-list] [product-price])')
                        .append(
                          '<span class="shb-old-price" product-price-old><span>' +
                            Shop.money_format.replace('{{amount}}', priceRegular.toFixed(2)) +
                            '</span></span>'
                        );
                    }
                  } else {
              ...
              if (v.id == variantId) {
                ...
                if (price != priceRegular) {
                  if ($product.find('.shb-product-list-price .shb-product-list-old-price').length == 0) {
                    ...
                    $product
                      .find('.shb-product-list-price')
                      .append(
                        '<span class="shb-product-list-old-price">' +
                          Shop.money_format.replace('{{amount}}', priceRegular.toFixed(2)) +
                          '</span>'
                      );
                    $product
                      .find('.shb-product-list-price')
                      .append(
                        '<span class="shb-product-list-new-price">' +
                          Shop.money_format.replace('{{amount}}', price.toFixed(2)) +
                          '</span>'
                      );
                  } else {
                    $product
                      .find('.shb-product-list-price .shb-product-list-new-price')
                      .text(Shop.money_format.replace('{{amount}}', price.toFixed(2)));
                  }
                } else {
                  $product
          ...
          $.ajax({
            ...
            $.each(data.variants, function(k, v) {
              if (v.id == variantId) {
                ...
                $('body')
                  .find(`[cart-product][data-variant-id='${variantId}']`)
                  .find('[product-quantity]')
                  .attr('data-price', price.toFixed(2));
                $('body')
                  .find(`[cart-product][data-variant-id='${variantId}']`)
                  .find('[product-price]')
                  .text(Shop.money_format.replace('{{amount}}', price.toFixed(2)));
              ...
              $.each(data.items, function(i, val) {
                ...
                if (itemTemplate.length) {
                  ...
                  if (val.price_original && price < priceOriginal) {
                    $(
                      '<span product-price-original class="shb-item-price shb-item-price-old">' +
                        Shop.money_format.replace('{{amount}}', priceOriginal) +
                        '</span>'
                    ).insertBefore(newItem.find(gs.priceText));
                  }
                  ...
                  cartItem
                    .find('[product-price]')
                    .not('.product-price-sum')
                    .text(Shop.money_format.replace('{{amount}}', price));
                }
              });
              cartOptions.productCartEvents();
            } else {
        ...
        sumAllProducts: function() {
          ...
          $.each($(gs.products), function() {
            ...
            if ($(this).find(gs.priceText).length && typeof productPrice !== 'undefined') {
              $(this)
                .find(gs.priceText)
                .text(Shop.money_format.replace('{{amount}}', productPrice.toFixed(2)));
            }

            if ($(this).find(gs.productPriceSumWrapper).length) {
              $(this)
                .find(gs.productPriceSumWrapper)
                .text(Shop.money_format.replace('{{amount}}', productOverall.toFixed(2)));
            }
          });
          var priceWithoutDelivery = parseFloat(overall).toFixed(2);
          if ($('[cart-sum-without-shipping]').length) {
            $('[cart-sum-without-shipping]')
              .find('p')
              .text(Shop.money_format.replace('{{amount}}', priceWithoutDelivery));
          }
          if (
            gs.showDefaultDeliveryCost &&
            ((gs.showFreeShippingInfo && parseFloat(overall) < parseFloat(cart_free_delivery_price)) ||
              !gs.showFreeShippingInfo)
          ) {
            ...
            if (data.status === 'ok') {
              ...
              if (gs.isDynamic) {
                if ($('body').find(`.product-list input[data-product-id='${id}']`).length) {
                  ...
                  var thisId = $('body#page-product')
                      .find('form input[data-product-id]')
                      .attr('data-product-id'),
                    thisVariantId = $('body#page-product')
                      .find('form input[data-product-id]')
                      .val();
        ...
        ajaxClearCart: function() {
          SHOPLOAJAX.clearCart().done(function(data) {
        ...
        updateFreeShippingInfo: function(total_price) {
          ...
          if (cartTotalPrice < priceToFreeShipping) {
            var priceToGetFree = priceToFreeShipping - cartTotalPrice,
              message = cart_free_delivery_info.replace(
                '{price}',
                '<b>' + Shop.money_format.replace('{{amount}}', priceToGetFree.toFixed(2)) + '</b>'
              );
            ...
            if (freeShippingMessage.length) {
              ...
              if ($('body').find('.shb-cart-free-shipping-info-wrapper').length) {
                $('body')
                  .find('.shb-cart-free-shipping-info-wrapper')
                  .removeClass('is-hidden')
                  .append(infoBlock.fadeIn());
              } else {
