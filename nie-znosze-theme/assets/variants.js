...
(function($) {
    ...
    var s,
      ...
      ProductVariants = {
        settings: {
          ...
          options: $(gs.productSectionContainer)
            .find(gs.variantsContainer)
            .find(gs.propertyContainer),
        ...
        setVariant: function() {
          $('.shb-product-actions')
            .find('input[name="id"]')
            .attr('data-sku', s.currentVariant.sku);
          ...
          if (productsOptionsWrapper.length && additionalProperties.length) {
            additionalProperties.each(function() {
              var newName = $(this)
                .attr('name')
                .replace(/properties\[.*?\]/, 'properties[' + s.currentVariant.id + ']');
        ...
        setNewPrice: function() {
          ...
          if (productIdPo.length) {
            productIdPo
              .attr('data-price', s.currentVariant.price)
              .attr('data-regular-price', s.currentVariant.priceRegular);
            Shop.productOptions.setNewPrice();
          }
          $('.shb-product-price').removeClass('is-invisible');
        },
            ...
            for (i = 0; i < thumb.length; i += 1) {
              if (s.thumbsWrapper.length) {
                if (
                  $(thumb[i])
                    .find('img')
                    .attr('src') === s.currentVariant.image
                ) {
                  $(thumb[i])
                    .find('img')
                    .trigger('click');
        ...
        initForm: function() {
          ...
          if (Object.keys(urlParams).length) {
            paramProperty = Object.keys(urlParams).some(function(k) {
              return ~k.indexOf('property');
            });
          ...
          if (s.currentVariant == null) ProductVariants.deactivateAllProperties();
        },
  ...
  $.fn.productVariants.defaults = {
    ...
    shippingInfoWrapper: '.shb-product-shipping-info',
    shippingInfo: 'span',
